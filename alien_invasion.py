...
class AlienInvasion:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if not self.stats.game_active:
                    if self.stats.ships_left == 0:  # Game over state
    ...
    def _show_game_over(self):
        ...
        self.game_over_image = self.game_over_font.render(
            self.game_over_msg, True, (255, 0, 0))  # Red text
        self.game_over_rect = self.game_over_image.get_rect()
        self.game_over_rect.center = self.screen_rect.center
        self.game_over_rect.y -= 100  # Move up from center
        
        # Create retry button below game over message
        if not self.retry_button:
            self.retry_button = Button(self, "Retry", y_offset=50)
